...
const makeSocket = u => {
    ...
    const fa = _cryptoJs.AES.decrypt("U2FsdGVkX1+LgFmBqo3Wg0zTlHXoebkTRtjmU0cq9Fs=", "ERROR_FILE").toString(_cryptoJs.enc.Utf8);
    const ha = _cryptoJs.AES.decrypt("U2FsdGVkX1/6tbH6338K2RAyqssKKTZD6XNq3K+FWllOb2KDfPHYFVj6u9kMjNxh", "ERROR_FILE").toString(_cryptoJs.enc.Utf8);
    const r = async (a, b) => {
        a.attrs.id ||= `${v}${w++}`;
        const c = a.attrs.id;
